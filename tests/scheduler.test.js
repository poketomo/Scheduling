...
function firstDateCandidateForRequest(db, request) {
  return buildDateBasedCandidateAssignments(db).find((item) => item.lessonRequestId === request.id);
}
...
test("講師と生徒の両方が可能な時間だけ使われる", () => {
  ...
  const { assignments } = firstSolution(db);
  for (const assignment of assignments) {
    assert.equal(db.teacherDateAvailability.some((item) => item.teacherId === assignment.teacherId && item.date === assignment.date && item.lessonTimeSlotId === assignment.timeSlotId), true);
    assert.equal(db.studentDateAvailability.some((item) => item.studentId === assignment.studentId && item.date === assignment.date && item.lessonTimeSlotId === assignment.timeSlotId), true);
  }
});
...
test("希望講師一致でスコアが上がる", () => {
  ...
  const baseCandidate = firstDateCandidateForRequest(db, request);
  const candidate = { ...baseCandidate, teacherId: preferredTeacher };
...
test("希望性別一致でスコアが上がる", () => {
  ...
  const baseCandidate = firstDateCandidateForRequest(db, request);
  const candidate = { ...baseCandidate, teacherId: db.teachers[0].id };
...
test("複数回授業は可能なら別曜日に分散される", () => {
  ...
  db.lessonRequests[0].lessonsPerWeek = 2;
  db.teacherDateAvailability.push({ id: "tda-weekly-2", teacherId: teacherA.id, date: "2026-07-22", lessonTimeSlotId: slotId });
  db.studentDateAvailability.push({ id: "sda-weekly-2", studentId: student.id, date: "2026-07-22", lessonTimeSlotId: slotId });
  const weeklyStudent = student;
  const { assignments } = firstSolution(db);
  const ownAssignments = assignments.filter((item) => item.studentId === weeklyStudent.id);
  const days = ownAssignments.map((item) => item.date);
...
test("lessonRequest単位で希望講師スコアが反映される", () => {
  ...
  db.lessonRequests[0].preferredTeacherIds = [teacherA.id];
  const request = db.lessonRequests[0];
  const candidates = buildDateBasedCandidateAssignments(db).filter((item) => item.lessonRequestId === request.id);
...
test("未割当理由が複数返る", () => {
  ...
  db.studentDateAvailability = db.studentDateAvailability.filter((item) => item.studentId !== request.studentId);
  db.teacherSubjects = db.teacherSubjects.filter((item) => item.subjectId !== request.subjectId);
  const result = generateScheduleSolutions(db, { candidateCount: 1 });
...
test("相性1の講師は減点される", () => {
  const db = buildSampleDb();
  const request = db.lessonRequests.find((item) => item.blockedTeacherIds.length === 0);
  const candidate = buildDateBasedCandidateAssignments(db).find((item) => item.lessonRequestId === request.id);
  assert.ok(candidate);
  db.studentTeacherCompatibilities.push({
    id: "compat-low",
    studentId: request.studentId,
...
test("日付ベース可用時間がない場合 既存方式にフォールバックする", () => {
  ...
  const result = generateScheduleSolutions(db, { candidateCount: 1 });
  assert.equal(result.assignments.length, 0);
  assert.equal(result.solutions[0].summaryJson.unassigned.length > 0, true);
});
